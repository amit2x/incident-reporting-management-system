...
@push('styles')
<style>
    .timeline-item { position: relative; padding-left: 32px; margin-bottom: 16px; }
    .timeline-item::before { content: ''; position: absolute; left: 11px; top: 28px; bottom: -16px; width: 2px; background: #e5e7eb; }
    .timeline-item:last-child::before { display: none; }
    .timeline-dot { position: absolute; left: 0; top: 2px; width: 24px; height: 24px; border-radius: 50%; display: flex; align-items: center; justify-content: center; }
    .comment-item { transition: background 0.2s; }
    .comment-item:hover { background: #f9fafb; }
    .media-thumb { cursor: pointer; transition: transform 0.2s; border-radius: 8px; }
    .media-thumb:hover { transform: scale(1.02); }
    .comment-attachment { display: inline-flex; align-items: center; gap: 4px; padding: 4px 10px; background: #f3f4f6; border-radius: 8px; font-size: 0.75rem; }
</style>
@endpush
...
<div class="container-fluid px-3 py-3">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-start mb-3">
        ...
        <div class="d-flex flex-wrap gap-2 mt-2 mt-md-0">
            {{-- Share Button --}}
            <button class="btn btn-outline-info btn-sm" onclick="shareIncident()">
                <i class="fas fa-share-alt"></i> <span class="d-none d-md-inline ms-1">Share</span>
            </button>

            @can('edit-incident')
                <a href="{{ route('incidents.edit', $incident) }}" class="btn btn-outline-secondary btn-sm"><i class="fas fa-edit"></i> Edit</a>
            @endcan

            {{-- Reopen (only if resolved/closed) --}}
            @if(in_array($incident->status, ['resolved', 'closed']))
                @can('reopen-incident')
                <button class="btn btn-outline-primary btn-sm" onclick="reopenIncident()"><i class="fas fa-redo"></i> Reopen</button>
                @endcan
            @endif

            {{-- Assign/Reassign --}}
            @can('assign-incident')
                <button class="btn btn-outline-primary btn-sm" data-bs-toggle="modal" data-bs-target="#assignModal"><i class="fas fa-user-plus"></i> {{ $incident->assignedTo ? 'Reassign' : 'Assign' }}</button>
            @endcan

            {{-- Reject (only if open/acknowledged) --}}
            @if(in_array($incident->status, ['open', 'acknowledged']))
                @can('close-incident')
                <button class="btn btn-outline-danger btn-sm" data-bs-toggle="modal" data-bs-target="#rejectModal"><i class="fas fa-times-circle"></i> Reject</button>
                @endcan
            @endif

            @can('escalate-incident')
                <button class="btn btn-outline-warning btn-sm" data-bs-toggle="modal" data-bs-target="#escalateModal"><i class="fas fa-arrow-up"></i> Escalate</button>
            @endcan

            @can('resolve-incident')
                @if(!in_array($incident->status, ['resolved', 'closed', 'rejected']))
                <button class="btn btn-outline-success btn-sm" data-bs-toggle="modal" data-bs-target="#resolveModal"><i class="fas fa-check-circle"></i> Resolve</button>
                @endif
            @endcan

            @can('close-incident')
                @if(!in_array($incident->status, ['closed', 'rejected']))
                {{-- <button class="btn btn-outline-dark btn-sm" onclick="closeIncident()"><i class="fas fa-lock"></i> Close</button> --}}
                <button class="btn btn-outline-dark btn-sm" data-bs-toggle="modal" data-bs-target="#closeModal">Close</button>
    ...
    <div class="row g-3">
        ...
        <div class="col-lg-8">
            ...
            <div class="d-flex flex-wrap gap-2 mb-3">
                <span class="badge status-{{ str_replace('_', '-', $incident->status) }}">{{ str_replace('_', ' ', ucfirst($incident->status)) }}</span>
                <span class="badge priority-{{ $incident->priority }}"><i class="fas fa-flag me-1"></i>{{ ucfirst($incident->priority) }}</span>
                <span class="badge" style="background:{{ $incident->severity_color }}15;color:{{ $incident->severity_color }}"><i class="fas fa-exclamation-circle me-1"></i>{{ ucfirst($incident->severity) }}</span>
                @if($incident->is_overdue)<span class="badge bg-danger"><i class="fas fa-clock me-1"></i>Overdue</span>@endif
            </div>

            {{-- Description --}}
            <div class="card mb-3 shadow-sm">
                <div class="card-header bg-white"><strong><i class="fas fa-align-left text-primary me-2"></i>Description</strong></div>
                <div class="card-body">
                    <p class="mb-0" style="white-space: pre-wrap;">{{ $incident->description }}</p>
                    @if($incident->tags)
                        <div class="mt-3">@foreach($incident->tags as $tag)<span class="badge bg-light text-dark me-1">#{{ $tag }}</span>@endforeach</div>
                    @endif
                </div>
            </div>
                        ...
                        @if($incident->resolution_notes)
                            <div class="col-12 border-top pt-3 mt-1">
                                ...
                                <div class="bg-light rounded p-3">
                                    <span class="fw-medium" style="white-space: pre-wrap;">{{ $incident->resolution_notes }}</span>
            ...
            <div class="card mb-3 shadow-sm">
                <div class="card-header bg-white"><strong><i class="fas fa-paperclip text-success me-2"></i>Attachments ({{ $incident->media->count() }})</strong></div>
                <div class="card-body"><div class="row g-2">@foreach($incident->media as $media)<div class="col-4 col-md-3 col-lg-2">@if($media->isImage())<img src="{{ $media->url }}" class="img-fluid rounded media-thumb" style="width:100%;height:100px;object-fit:cover;" onclick="window.open('{{ $media->url }}','_blank')">@else<a href="{{ $media->url }}" target="_blank" class="text-decoration-none"><div class="border rounded p-2 text-center" style="height:100px;"><i class="fas fa-file fa-2x text-muted mt-2"></i><small class="d-block text-truncate mt-1">{{ $media->original_name }}</small></div></a>@endif</div>@endforeach</div></div>
            </div>
            @endif
            ...
            <div class="card shadow-sm" id="commentsSection">
                <div class="card-header bg-white d-flex justify-content-between align-items-center">
                    <strong><i class="fas fa-comments text-primary me-2"></i>Comments (<span id="commentCount">{{ $incident->comments_count ?? 0 }}</span>)</strong>
                </div>
                <div class="card-body">
                    {{-- Comment Form --}}
                    <form id="commentForm" class="mb-4">
                        ...
                        <div class="d-flex gap-2 mb-2">
                            <img src="{{ Auth::user()->avatar_url }}" class="rounded-circle flex-shrink-0" width="36" height="36" style="object-fit:cover;">
                            <div class="flex-grow-1">
                                ...
                                <div class="d-flex justify-content-between align-items-center mt-2">
                                    <div class="d-flex align-items-center gap-2">
                                        <button type="button" class="btn btn-light btn-sm attach-comment-btn" id="attachCommentBtn" title="Attach files">
        ...
        <div class="col-lg-4">
                            ...
                            <div class="list-group-item small">
                                <div class="d-flex justify-content-between align-items-start">
                                    <div>
                                        ...
                                        @endif
                                        <small class="text-muted">By: {{ $assignment->assignedBy?->name ?? 'System' }}</small>
                                    </div>
                                    <small class="text-muted flex-shrink-0">{{ $assignment->created_at->format('d M H:i') }}</small>
                                </div>
                                @if($assignment->unassigned_at)
                                    <small class="text-danger">Unassigned: {{ $assignment->unassigned_at->format('d M H:i') }}</small>
                                @endif
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="card shadow-sm">
                <div class="card-header bg-white"><strong><i class="fas fa-info-circle me-2"></i>Quick Info</strong></div>
                <div class="card-body small">
                    <div class="row g-2">
                        <div class="col-6"><strong>Response:</strong><br><span class="text-muted">{{ $incident->response_time ?? 'N/A' }}</span></div>
                        <div class="col-6"><strong>Resolution:</strong><br><span class="text-muted">{{ $incident->resolution_time ?? 'N/A' }}</span></div>
                        <div class="col-6"><strong>SLA Breaches:</strong><br><span class="{{ $incident->sla_breach_count > 0 ? 'text-danger' : 'text-muted' }}">{{ $incident->sla_breach_count }}</span></div>
                        <div class="col-6"><strong>Views:</strong><br><span class="text-muted">{{ $incident->views_count }}</span></div>
                        <div class="col-6"><strong>Escalations:</strong><br><span class="{{ $incident->escalations->count() > 0 ? 'text-warning' : 'text-muted' }}">{{ $incident->escalations->count() }}</span></div>
                        <div class="col-6"><strong>Reassignments:</strong><br><span class="text-muted">{{ max(0, $incident->assignmentHistory()->count() - 1) }}</span></div>
                    </div>
                </div>
            </div>
        </div>
